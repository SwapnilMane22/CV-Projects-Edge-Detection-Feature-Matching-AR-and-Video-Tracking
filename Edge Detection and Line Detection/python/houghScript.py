...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../ec')))

from myHoughLineSegments import myHoughLineSegments #myHoughLinesP, 
...
threshold = 0.03 #0.03
rhoRes    = 2 #2
thetaRes  = np.pi / 90 #90
nLines    = 20 #15
minLineLength = 20  # Minimum length of a line segment
maxLineGap = 5      # Maximum gap between segments to connect them
# end of parameters
...
for file in os.listdir(datadir):
    if file.endswith('.jpg'):
        ...
        # my_lines = myHoughLineSegments(rhos, thetas, img_threshold)
        my_img_lines = np.dstack([img, img, img])
        my_lines, my_img_lines = myHoughLineSegments(rhos, thetas, img_threshold, my_img_lines)
        ...
        for k in np.arange(nLines):
            ...
            cv2.line(my_img_lines,(x1,y1),(x2,y2),(0,0,255),1)
        ...
        # Save the final image with lines drawn
        fname = '%s/%s_myHoughLineSegments.png' % (resultsdir, file)
        cv2.imwrite(fname, 255 * my_img_lines)