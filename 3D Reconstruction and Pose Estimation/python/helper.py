...
def normalize_points(x):
    ...
    translation = np.mean(x, axis=0)

    # Shift the origin of the points to the centroid
    x -= translation

    # Normalize the points so that the largest distance from the origin is equal to sqrt(2)
    scale = np.sqrt(2) / np.max(np.hypot(*x.T))
    x *= scale

    # Similarity transform
    scale_transform = np.diag([scale, scale, 1])
    translate_transform = np.diag([1, 1, 1])
    translate_transform[:, 2] = np.append(-translation, 1)
    transform = scale_transform @ translate_transform

    return x, transform


def reprojection_error(pts3d, pts, P):
    pts3d_h = np.hstack((pts3d, np.ones((pts3d.shape[0], 1))))

    pts_reproject_h = (P @ pts3d_h.T).T
    pts_reproject = pts_reproject_h[:, :-1] / pts_reproject_h[:, -1, np.newaxis]
    error = np.mean(np.linalg.norm(pts - pts_reproject, axis=1))
...
def displayEpipolarF(I1, I2, F):
    ...
    while True:
        plt.sca(ax1)
        points = plt.ginput(1, mouse_stop=2)

        # Check if any points were clicked
        if not points:
            # print("No points selected or input interrupted. Exiting...")
            break 
...
def warpStereo(I1, I2, M1, M2):
    bb = _mcbbox(I1.shape, I2.shape, M1, M2)
    shift = np.array([[1, 0, -bb[0]], [0, 1, -bb[1]], [0, 0, 1]])

    I1p = _imwarp(I1, shift @ M1, bb)
    I2p = _imwarp(I2, shift @ M2, bb)

    return I1p, I2p, bb, shift